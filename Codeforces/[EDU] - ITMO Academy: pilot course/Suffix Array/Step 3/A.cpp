...
bool bb(vector<int> &arr, string &s, string &t){
    int l = 0, r = arr.size()-1, med, answ=-1;
    ...
    bool poss = (answ!=-1 && s.size()-answ >= t.size());
    for(int i=0; poss && i<t.size();i++)
        if(t[i]!=s[answ+i])
            poss = false;
