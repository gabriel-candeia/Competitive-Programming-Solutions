...
struct minqueue{
    int id = 0, removidos = 0;
    deque<pair<int,int>> q;

    void push(int x){
        pair<int,int> novo = {x,id++};
        while(q.size()!=0 && q.back().first>novo.first){
            q.pop_back();
        }
        q.push_back(novo);
    }
    ...
    int segundo(){
        return q[(removidos==q.front().second)].first;
    }
};
...
int main(){
    ...
    while(t--){
        ...
        q.push_back(n-sa.sa[0]);
        mn.push(sa.lcp[0]);
        for(int i=1;i<sa.sa.size();i++){
            while(q.size()>1 && q[0]-2*mn.front() <= q[1]-2*mn.segundo()){
                q.pop_front();
                mn.pop();
            }
            answ = max(answ,n-sa.sa[i]+q[0]-2*mn.front());
            q.push_back(max(q.back(),n-sa.sa[i]));
            mn.push(sa.lcp[i]);
        }
